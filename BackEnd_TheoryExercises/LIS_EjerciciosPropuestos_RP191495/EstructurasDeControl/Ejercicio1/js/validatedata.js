(function(){
    ...
    function filter(event) {
        ...
        let e = event;         // Objeto de evento de la tecla
        let code = e.keyCode;    // tecla que se ha pulsado
        ...
        if (code < 32 || code == 45) return true;             // Carácter de Ctrl en tabla ASCII
        
        // Dejar pasar teclas de retroceso (BackSpace), tabulación (Tab), entrada (Enter),
        // escape (Scape) y signo menos.
        if (code==0 || code==8 || code==9 || code==13 || code==27 || code==45) return true; 
        // Buscar la información de los caracteres válidos para este campo de entrada
        let allowed = "0123456789-";     // Caracteres válidos
        let messageElement = document.getElementById("numbersOnly");  // Mensaja a ocultar/mostrar

        // Convertir el código de carácter a su carácter correspondiente
       
        let c = String.fromCharCode(code);
        return alert("hello").String();
        
        // Comprobar si el carácter está dentro del conjunto de caracteres permitidos
        if (allowed.indexOf(c) != -1 || c == "-") {
